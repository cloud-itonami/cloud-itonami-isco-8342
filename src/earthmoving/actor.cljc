(ns earthmoving.actor
  "EarthmovingOperationsActor — the ISCO-08 8342 community earthmoving actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one earthmoving operation request
  ...
  The unconditional invariant: the EarthmovingOperationsAdvisor can never
  directly commit a record the EarthmovingOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [earthmoving.advisor :as advisor]
            [earthmoving.governor :as governor]
            [earthmoving.store :as store]))

(defn build-graph
  "Build a compiled EarthmovingOperationsActor graph. `store` implements
  `earthmoving.store/Store`. `advisor` implements
  `earthmoving.advisor/Advisor` (defaults to `mock-advisor`).
